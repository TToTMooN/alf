...
"""Multi task Soft Actor Critic Algorithm."""
...
@alf.configurable
class MTSacAlgorithm(OffPolicyAlgorithm):
    r"""Multi task Soft Actor Critic algorithm, described in:
    ...
    def predict_step(self, inputs: TimeStep, state: SacState):
        ...
        if alf.summary.render.is_rendering_enabled():
            ...
            # # test phase manual change on weight matrix
            # with torch.no_grad():
            #     custom_fc_weight = torch.tensor([[-0.17364818, -0.98480775],
            #                                      [-0.17364818, -0.98480775],
            #                                      [0.58951306, 0.8077588]]).T
            #     self._actor_network._task_encoding_net._fc_layers[
            #         -1]._weight.copy_(custom_fc_weight)
    ...
    def after_update(self, root_inputs, info: SacInfo):
        ...
        if self._max_log_alpha is not None:
            nest.map_structure(
                ...
                self._log_alpha)
        # reset task weight in extreme task sets
        if self._reset_task_while_extreme and self._extreme_task_mask is not None:
            ...
            for fc in self._task_encoding_network._fc_layers:
                ...
                normal_task_weight = new_weight[:,
                                                normal_task_mask]  # 5, num_normal
                interpolate_weight = torch.rand(
                    (extreme_task_set.shape[0],
                     normal_task_set.shape[0]))  # num_extreme, num_of_normal
    ...
    def calc_loss(self, info: SacInfo):
        ...
        task_reg_loss = self._reg_weight * (
            diversity_loss + 0.0 * sparsity_loss + 0.0 * coverage_loss)
